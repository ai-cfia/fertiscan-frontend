...
const InspectionElement = ({inspection, handleClick}: InspectionElementProps) => {
  return <Card className={"p-2 xs:h-[75px] s:h-[100px] cursor-pointer hover:shadow-xl flex-shrink-0"} onClick={handleClick}>
    <Stack direction={"row"} className={"h-full items-center"}>
      ...
      {inspection.updated_at===inspection.upload_date && <ErrorIcon data-testid={"error-icon"} color="error"></ErrorIcon>}
    </Stack>
  </Card>
}
