...
const LanguageButton = () => {
  ...
  const getIcon = () => {
    switch (language) {
      case 'en':
        return <i className="flag-icon flag-icon-ca"></i>; // Canada flag icon for English
      case 'fr':
        return <i className="flag-icon flag-icon-ca-fr"></i>; // Canada flag icon for French
      default:
        return null;
    }
  };
  ...
  return (
    <button className={`language-button ${language}`} onClick={handleLanguageSwap}>
      {getIcon()} {buttonText}
    </button>
  );
};
